...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname 155-inner) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
; RD -> String
; gets the color of the innermost doll
(check-expect (inner (make-layer "yellow" (make-layer "green" "red"))) "red")
(define (inner an-rd)
  (cond
    [(string? an-rd) an-rd] ; with one doll, the innermost doll is itself
    [(layer? an-rd) (inner (layer-doll an-rd))])) ; go one level deeper