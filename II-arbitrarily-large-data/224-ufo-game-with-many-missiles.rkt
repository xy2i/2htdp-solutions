...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname 224-ufo-game-with-many-missiles) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define-struct fired [ufo tank lom])
; A SIGS is one of: 
; – (make-aim UFO Tank)
; – (make-fired UFO Tank List-of-Missile)
...
(define s3 (make-fired (make-posn 20 100)
                       (make-tank 100 3)
                       (list (make-posn 22 103) (make-posn 24 130))))
(define e3 (place-image UFO 20 100
                        (place-image MISSILE 22 103
                                     (place-image MISSILE 24 130
...
(define (missile-render lom im)
  (cond
    [(empty? (rest lom)) ; only one missile
     (place-image MISSILE (posn-x (first lom)) (posn-y (first lom)) im)]
    [else ; more than one
     (missile-render (rest lom)
                     (place-image MISSILE (posn-x (first lom)) (posn-y (first lom)) im))]))
...
(check-expect (si-game-over? (make-fired (make-posn 10 190) (make-tank 24 0) (list (make-posn 0 0)))) #true)
(check-expect (si-game-over? (make-fired (make-posn 10 10) (make-tank 24 0) (list (make-posn 7 10)))) #true)

(define (si-game-over? s)
  (cond
    [(aim? s) (ufo-land? (aim-ufo s))]
    [(fired? s) (or (close-missiles? (fired-ufo s) (fired-lom s))
                    (ufo-land? (fired-ufo s)))]))

; Posn List-of-missiles -> Boolean
; tests each missile for proximity
(define (close-missiles? u lom)
  (cond
    [(empty? lom) #false]
    [else
     (or (close? u (first lom))
         (close-missiles? u (rest lom)))]))
...
(check-expect (si-move-proper (make-fired (make-posn 40 20) (make-tank 24 0) (list (make-posn 7 40)))
                              12)
              (make-fired (make-posn 52 23) (make-tank 24 0) (list (make-posn 7 34))))
...
; moves a list of missiles
(check-expect (move-missiles (list (make-posn 7 40))) (list (make-posn 7 34)))
(check-expect (move-missiles (list (make-posn 0 100))) (list (make-posn 0 94)))
(check-expect (move-missiles (list (make-posn 7 40) (make-posn 0 6)))
              (list (make-posn 7 34) (make-posn 0 0)))
...
(check-expect (si-control (make-aim (make-posn 20 10) (make-tank 28 -3)) " ")
              (make-fired (make-posn 20 10) (make-tank 28 -3) (list (make-posn 28 (- HEIGHT TANK-HEIGHT)))))
...
(check-expect (direct-tank (make-aim (make-posn 20 10) (make-tank 28 -3))
                           "left")
              (make-aim (make-posn 20 10) (make-tank 28 -10)))
(check-expect (direct-tank (make-fired (make-posn 20 10) (make-tank 28 -3) (list (make-posn 28 (- HEIGHT TANK-HEIGHT))))
                           "left")
              (make-fired (make-posn 20 10) (make-tank 28 -10) (list (make-posn 28 (- HEIGHT TANK-HEIGHT)))))
(check-expect (direct-tank (make-aim (make-posn 20 10) (make-tank 28 -3))
                           "right")
              (make-aim (make-posn 20 10) (make-tank 28 10)))
(check-expect (direct-tank (make-fired (make-posn 20 10) (make-tank 28 -3) (list (make-posn 28 (- HEIGHT TANK-HEIGHT))))
...
; SIGS -> Fired
; fires a missile, or adds one to the list if some are already fired
(check-expect (fire-missile (make-aim (make-posn 20 10) (make-tank 28 -3)))
              (make-fired (make-posn 20 10) (make-tank 28 -3) (list (make-posn 28 (- HEIGHT TANK-HEIGHT)))))
(check-expect (fire-missile (make-fired (make-posn 20 10)
                                        (make-tank 28 -3)
                                        (list (make-posn 20 (- HEIGHT TANK-HEIGHT)))))
              (make-fired (make-posn 20 10)
                          (make-tank 28 -3)
                          (list (make-posn 28 (- HEIGHT TANK-HEIGHT))
                                (make-posn 20 (- HEIGHT TANK-HEIGHT)))))
(define (fire-missile s)
  (if (aim? s) ; we have an Aim, make it a Fired
      (make-fired (aim-ufo s)
                  (aim-tank s)
                  (list (make-posn (tank-loc (aim-tank s))
                                   (- HEIGHT TANK-HEIGHT))))
      (make-fired (fired-ufo s)
                  (fired-tank s)
                  (cons
                   (make-posn (tank-loc (fired-tank s))
                              (- HEIGHT TANK-HEIGHT))
                   (fired-lom s))))) ; add a missile to the list
